 ...
 *
  ...
  def trainingProcessWithRDD(train: RDD[Seq[Object]], test: RDD[Seq[Object]], parasDoc: Int, parasFeatrues: Int, writeModel:Boolean) = {
    ...
    val hashingTFModel = new feature.HashingTF(5000)
    val trainTFRDD = train.map(line => {
      val temp = hashingTFModel.transform(line(1).asInstanceOf[Seq[String]])
//      (if(line(1).asInstanceOf[String] == "881155") 1.0 else 0.0, temp)
      (line(0).toString.toDouble,temp)
    })
    ...
    val testRDD = test.map(line => {
      val temp = line(1).asInstanceOf[Seq[String]]
...
//      LabeledPoint(if(line(0).asInstanceOf[String] == "881155") 1.0 else 0.0, tempSelected)
      LabeledPoint(line(0).toString.toDouble, tempSelected)
    })
