...
object TextProcess {
  ...
  def process(content: String, stopWordsBr: Broadcast[Array[String]]): Array[String] = {
    ...
    val contentString = content.replaceAll("[\n\t\r\f\b ]", ",")
    //实现分词
    val splitWords = WordSeg.splitWord(contentString, 1)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val stopWords = Source.fromFile("/Users/li/kunyan/nlp/text_classification/1.1/Spark_NLP_suit/src/main/resources/dicts/stop_words_CN").getLines().toArray
    ...
    val sqlString = "SELECT url, content FROM stock.indus_text_with_label group by url limit 10"
    // 调用函数获取数据库中的数据
