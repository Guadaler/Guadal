...
object Bayes {
  ...
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("BayesPredictTest").setMaster("local")
    val sc = new SparkContext(conf)
    val modelMap = initModel("/mlearning/Models/", hdfs = true)
    ...
    val cateString = industryPredict(content, modelMap, stopWordsBr)
    println(cateString)
  }
  ...
  /**
    ...
    */
  def initModel(path: String, hdfs: Boolean): Map[String, Map[String, AnyRef]] = {
    ...
    if (hdfs){
      ...
      val resultMap = fs.listStatus(new Path(path)).map(_.getPath.toString).map(filePath => {
        val indus = filePath.replaceAll("hdfs://222.73.57.12:9000/mlearning/Models/", "")
      ...
      val resultMap = cateList.map(cate => {
